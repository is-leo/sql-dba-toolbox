...
SELECT [sch].[name] + '.' + [so].[name] as table_name, [so].[object_id], [ss].[name], [ss].[stats_id]
FROM [sys].[stats] [ss]
...
/*
	View stat header info easily with new DMF , check how many modification were made
*/
SELECT *
FROM [sys].[dm_db_stats_properties](1490104349,3);
