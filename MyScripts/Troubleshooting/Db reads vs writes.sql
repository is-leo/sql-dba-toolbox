...
<documentation>
  ...
  <modified>2024-02-29 by Leo Ismatov: encounted: 'Divide by zero error encountered' issue when all_activity returned 0.
  Added Case clause </modified>
  <version>1.1</version>
...
SELECT
    database_name
    , reads
    , CASE WHEN all_activity = 0 THEN 0 ELSE ((reads * 1.0) / all_activity) * 100 END AS reads_percent
    , writes
    , CASE WHEN all_activity = 0 THEN 0 ELSE ((writes * 1.0) / all_activity) * 100 END AS writes_percent
