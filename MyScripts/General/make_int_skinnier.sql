...
(
  ...
  ) AS v(seq,src,trg,type_id,[type],bytes,minval_in_column,maxval_in_column)
),
cols AS
(
  SELECT t.[object_id],
         ...
         trgtyp.minval_in_column,
         trgtyp.maxval_in_column,
...
DECLARE @sql nvarchar(max) = N';WITH x([object_id], [column], minval_in_column, maxval_in_column)
AS (',
        @core nvarchar(max) = N'
...
SET @sql += N'
  ...
  SELECT c.[schema], c.[table], c.[column], c.is_nullable,
    ...
    space_savings_bytes = c.savings * c.[rowcount], x.minval_in_column, x.maxval_in_column,
    range = RTRIM(c.minval_in_column) + '' -> '' + RTRIM(c.maxval_in_column) 
  FROM x
  INNER JOIN #cols AS c 
  ON x.[object_id] = c.[object_id]
  AND x.[column] = c.[column]
  AND x.minval_in_column >= c.minval_in_column
  AND x.maxval_in_column <= c.maxval_in_column;';
